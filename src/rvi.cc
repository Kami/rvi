...
#define RC_CHECK_SCRIPT "rc_conf_syntax_check"
...
int check_for_rc_conf_syntax_errors();
int dialog();

int main(int argc, char ** argv)
{
	printf(
			"Revision Control System Interface (rvi) <mtsegaye-fm@rucus.ru.ac.za>\n");
	if (argc < 2)
	{
...
void do_filecheck(char * filename)
{
	...
	if (!((filecheck->st_mode - S_IFDIR | S_IWUSR) == filecheck->st_mode
			- S_IFDIR))
	{
	...
	{
		printf(
				"%s isn't being handled by RCS, check it into RCS for the first time[N] ",
				filename);
		if (dialog() != DG_YES)
			exit(0);
#ifndef DEBUG
	...
	// check if file has been checked out aready/Writable version exists
	{
		memset(filecheck, -1, sizeof(struct stat));
		stat(filename, filecheck);
		if ((filecheck->st_mode - S_IFREG | S_IWUSR) == filecheck->st_mode
				- S_IFREG || (filecheck->st_mode - S_IFREG | S_IWGRP)
				== filecheck->st_mode - S_IFREG || (filecheck->st_mode
				- S_IFREG | S_IWOTH) == filecheck->st_mode - S_IFREG)
		{
...
int do_filechange_check(char * fname, struct stat * original)
{
	...
	{
		// Editing a rc.conf file, check for syntax errors
		if (strcmp(fname, RC_CONF) == 0)
		{
			status = check_for_rc_conf_syntax_errors();

			// File contains syntax errors
			if (status == -1)
			{
				printf("Warning: File %s contains syntax errors!\n", fname);
			}
		}

		printf("You made changes to %s, (Accept \\ Re-edit \\ Discard) [A] ",
				fname);
		int key = dialog();
		if (key == ABORT)
...
// runs rc_conf_syntax_check.py Python script which checks if the /etc/rc.conf
// file contains any syntax errors
int check_for_rc_conf_syntax_errors()
{
	FILE *script;
	PyObject *p_name, *p_module, *p_func, *p_args, *p_return;

	Py_SetProgramName(RC_CHECK_SCRIPT);
	Py_Initialize();

	p_name = PyString_FromString(RC_CHECK_SCRIPT); // rc_conf_syntax_check.py must be in your Python path
	p_module = PyImport_Import(p_name);

	if (p_module != NULL)
	{
		p_func = PyObject_GetAttrString(p_module, "check_syntax");

		if (p_func && PyCallable_Check(p_func))
		{
			p_args = PyTuple_New(2);
			PyTuple_SetItem(p_args, 0, PyString_FromString(RC_CONF));
			PyTuple_SetItem(p_args, 1, PyBool_FromLong(0));

			p_return = PyObject_CallObject(p_func, p_args);

			// File contains syntax errors
			if (p_return == Py_False)
			{
				return -1;
			}

			// Everything is fine, no syntax errors found
			return 0;
		}
	}
